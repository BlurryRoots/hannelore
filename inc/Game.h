...
public:
	...
	on_update (double dt) {
		...
		glfonsRasterize (m_font_context, m_text_ids[1], ("frame: " + std::to_string (m_framecounter) /*+ "\nkeks mhh lecker!"*/).c_str ());
		// TODO: decide on how new lines are handled, ? parse string and create multiple lines ? just do not allow special characters
		//glfonsRasterize (m_font_context, m_text_ids[2], std::string("schnurpel\nkeks mhh lecker!").c_str ());
		glfonsRasterize (m_font_context, m_text_ids[2], std::string ("keks mhh lecker!").c_str ());
